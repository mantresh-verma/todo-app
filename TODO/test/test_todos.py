...
def test_read_one_authenticated(test_todo):
    response = client.get("user/todo/2")
...
def test_create_todo(test_todo):
    ...
    response = client.post("user/todo/add-todo/", json=request_data)
...
def test_update_todo(test_todo):
    ...
    response = client.put("user/todo/update-todo/1", json=request_data)
...
def test_update_todo_not_found(test_todo):
    ...
    response = client.put('user/todo/update-todo/10', json=request_data)
    assert response.status_code == 404
    assert response.json() == {'detail': 'Todo Not Found'}


def test_delete_one(test_todo):
    response = client.delete("user/todo/delete/1")
    assert response.status_code == status.HTTP_204_NO_CONTENT
    db = TestingSessionLocal()
    model = db.query(TODOS).filter(TODOS.id == 1).first()
...
def test_delete_not_found(test_todo):
    response = client.delete("user/todo/delete/2")
